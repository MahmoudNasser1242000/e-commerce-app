...
export default function WishListItem({imageCover, price, ratingsAverage, title, description, _id}) {
  ...
  const addProduct = async () => {
    ...
    if (data.status === "success") {
      notify = () => toast.success('Product added successfully', { icon: "👏", duration: 3000 });
      getCartItems();
      removeProduct();
      console.log(data, "success");
    } else {
      notify = () => toast.error("Can't add product!", { icon: "🔥", duration: 3000 });
    }
    return notify();
  }
      ...
      <div className="d-flex pb-5 justify-content-center align-items-center">
        <div className="container mx-auto row d-flex align-items-center">
          <div className="col-md-3 col-sm-6 mb-5 mb-sm-0">
            <img src={imageCover} className='w-100 object-fit-cover' height={300} alt={title} />
          </div>
          <div className="col-md-9 col-sm-6">
            <div>
              ...
              <div className="d-flex justify-content-between align-items-center py-4">
                <button className="btn btn-success rounded-1 w-75" onClick={addProduct}>Add To Cart +</button>
                <button className='btn btn-outline-danger rounded-1' onClick={removeProduct}><i className='fa fa-trash'></i></button>
              </div>
            </div>
          </div>
        </div>
